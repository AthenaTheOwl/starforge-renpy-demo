## combat_act1_boss_writbound.rpy — Narrative combat: Writbound Knight — Contract Enforcer
## Enemies: 1x Writbound Knight, 2x Lattice Shard
## Auto-generated skeleton — add flavor text

label combat_act1_boss_writbound:
    $ _nc = NarrativeCombat("act1_boss_writbound")
    show screen combat_hud(_nc)

    "BOSS ENCOUNTER. Elite Writbound enforcer bound by ancient contract. At half health, shield regeneration activates and Lattice shards manifest to defend the knight. Party split mechanic active."
...
label .combat_end:
    ...
    if _nc.victory:
        $ game_state.mark_encounter_cleared("act1_boss_writbound")
        $ party_manager.award_xp(500)
        $ game_state.set_flag("writbound_knight_defeated")
        $ game_state.set_flag("act1_final_boss_complete")
        ...
        $ game_state.set_flag("act1_boss_writbound_lost")
